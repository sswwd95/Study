...
x_train_noised = x_train_in + np.random.normal(0,0.1, size=x_train_in.shape)
...
x_test_noised = x_test_in + np.random.normal(0,0.1, size=x_test_in.shape)
...
def autoencoer(hidden_layer_size):
    model = Sequential()
    model.add(Conv2D(filters= hidden_layer_size, kernel_size=3, input_shape=(28,28,1), activation='relu'))
    model.add(Flatten())
...
model.fit(x_train_noised, x_train_out, epochs=10) # 노이즈 있는 것과 없는 것을 비교
