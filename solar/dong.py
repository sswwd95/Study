...
for i in quantiles:
    model = mymodel()
    filepath_cp = '../solar/check/dong_0121_day7_{epoch:02d}_{val_loss:.4f}.hdf5'
...
for i in quantiles:
    model = mymodel()
    filepath_cp = '../solar/check/dong_0121_day8_{epoch:02d}_{val_loss:.4f}.hdf5'
    cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
    model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
    model.fit(x_train,y2_train,epochs = epochs, batch_size = bs, validation_data = (x_val,y2_val),callbacks = [es,cp,lr])
    pred = round(pd.DataFrame(model.predict(x_test)),2)
    x.append(pred)
...
sub.to_csv('./solar/csv/dong_0121.csv', index=False)
