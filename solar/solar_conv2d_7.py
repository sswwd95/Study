...
target = target.reshape(int(target.shape[0]/48),1,48,target.shape[1])
...
for q in quantiles:
    model = Model()
    modelpath = '../solar/check/_conv2d_7_{epoch:02d}_{val_loss:.4f}.hdf5'
...
sub.to_csv('./solar/csv/sub_conv2d_7.csv',index=False)

loss_mean = sum(loss_list) / len(loss_list)
print('loss_mean : ', loss_mean)

# loss_mean :  2.4044533703062267 (스플릿 후 정규화)
# loss_mean :  2.4114996327294245 (정규화 후 스플릿)

# loss_mean :  2.3073587152693005 (y를 48로 스플릿)
