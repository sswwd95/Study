...
# filepath :  최저점마다 파일을 생성. 점마다 파일 생성하는건 낭비아닌가? 파일 안에 그 지점의 w값이 들어간다. predict, evaluate 할 때 파일에서 땡겨쓰면 좋다. 가장 마지막이 제일 값이 좋은 것.
hist = model.fit(x_train,y_train, callbacks=[es,cp], epochs=10, validation_split=0.2, batch_size=16)
