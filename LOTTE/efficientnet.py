...
mc = ModelCheckpoint('../data/lotte/mc/lotte_b7_sgd.h5',save_best_only=True, verbose=1)
...
# predict
model.load_weights('../data/lotte/mc/lotte_b7_sgd.h5')
result = model.predict(test_generator,verbose=True)
    
print(result.shape)
sub = pd.read_csv('../lotte/sample.csv')
sub['prediction'] = np.argmax(result,axis = 1)
sub.to_csv('../data/lotte/answer_b7_sgd.csv',index=False)
